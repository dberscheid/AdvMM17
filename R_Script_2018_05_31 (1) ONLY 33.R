...
# Warum sollte man das nochmal machen?
...
uniqzuie <- unique(datesFrame$week)
  
# 27. woche sollte july 4th sein 
1114 + 26  #1140

cola$July4th <- ifelse(cola$week==1140,1,
                       ifelse(cola$week==1192,1, 
                              ifelse(cola$week==1244,1, 
                                     ifelse(cola$week==1296,1, 
                                            ifelse(cola$week==1348,1, 
                                                   ifelse(cola$week==1400,1, 
                                                          ifelse(cola$week==1452,1, 
                                                                 ifelse(cola$week==1504,1, 
                                                                        ifelse(cola$week==1556,1,
                                                                               ifelse(cola$week==1608,1, 
                                                                                      ifelse(cola$week==1660,1,
                                                                                             0)))))))))))


cola$Halloween <- ifelse(cola$week==1157,1,
                       ifelse(cola$week==1209,1, 
                              ifelse(cola$week==1261,1, 
                                     ifelse(cola$week==1313,1, 
                                            ifelse(cola$week==1365,1, 
                                                   ifelse(cola$week==1417,1, 
                                                          ifelse(cola$week==1469,1, 
                                                                 ifelse(cola$week==1521,1, 
                                                                        ifelse(cola$week==1573,1,
                                                                               ifelse(cola$week==1625,1, 
                                                                                      ifelse(cola$week==1677,1,
                                                                                             0)))))))))))

cola$BlackFriday <- ifelse(cola$week==1160,1,
                         ifelse(cola$week==1212,1, 
                                ifelse(cola$week==1264,1, 
                                       ifelse(cola$week==1316,1, 
                                              ifelse(cola$week==1368,1, 
                                                     ifelse(cola$week==1420,1, 
                                                            ifelse(cola$week==1472,1, 
                                                                   ifelse(cola$week==1524,1, 
                                                                          ifelse(cola$week==1576,1,
                                                                                 ifelse(cola$week==1628,1, 
                                                                                        ifelse(cola$week==1680,1,
                                                                                                                   0)))))))))))

      
str(cola)
sapply(cola[,21:27 ],sum)   #change that once we have more variables
describe(cola)
...
# model.full<-lm(log(units)~log(price)+display+feature + PACKAGE ,data=cola)
# summary(model.full)
...
model<-lm(log(units)~log(price)+display+feature+yearasfactor+thanksgiving+christmas+newyearseve + July4th + Halloween + BlackFriday ,data=cola.sub)
summary(model)
...
cola.sub$sb <- ifelse(cola.sub$year>2006,1,0) # what does sb mean? look in documentation? maybe only to check if some changed in the second half of the decade
cola.sub$lagfeature<-lag(cola.sub$feature) #default is one lag #to see if the feature has an effect in the next period(?)


model<-lm(log(units)~log(price)*sb+display*sb+feature*sb+lagfeature*sb+yearasfactor+thanksgiving+christmas+newyearseve + July4th + Halloween + BlackFriday ,data=cola.sub)
summary(model)
# dwt(model)
# cor(cbind(cola.sub$display,cola.sub$feature))
# plot(x=cola.sub$week,y=residuals(model),xlab="Week", ylab="Residuals",panel.last = abline(h=0, lty=2))
